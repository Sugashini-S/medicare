...
import { ArrowRight, ShieldCheck, CheckCircle, Loader2 } from 'lucide-react';
...
const Login = () => {
  ...
  const handleVerify = async (otpData = otp) => {
    if (otpData.some(digit => !digit)) {
      import('react-hot-toast').then(toast => toast.default.error('Please enter all 6 digits'));
      return;
    }
    ...
    localStorage.setItem('spondon_loggedIn', 'true');
    const userData = { 
      phone, 
      name: 'User', // Default name for quick login
      isRegistered: true,
      hasFamilyPlan: false 
    };
    localStorage.setItem('spondon_user', JSON.stringify(userData));
    
    login({ phone }); // Still call login to update state
    import('react-hot-toast').then(toast => toast.default.success('Login successful!'));
    navigate('/dashboard');
  };
    ...
    <div className="min-h-screen bg-white pt-24 md:pt-32 pb-20 px-4 md:px-6 flex items-center justify-center">
      ...
      >
        <div className="text-center mb-10">
          <Link to="/" className="inline-flex items-center gap-3 mb-8 group">
            ...
            <div className="flex flex-col leading-none text-left">
              <span className="font-sans font-extrabold text-2xl" style={{ color: '#0d9488', letterSpacing: '0.05em' }}>SPONDON</span>
              <span className="font-sans font-medium text-xs text-text-muted">Doorstep Healthcare</span>
            </div>
          </Link>
          ...
          <AnimatePresence mode="wait">
            {step === 1 ? (
              ...
              >
                ...
                <p className="text-text-muted font-medium mb-8">Enter your registered mobile number</p>
                ...
                <form onSubmit={handleSendOTP} className="space-y-6">
                  ...
                  <button
                    ...
                    type="submit"
                    className="w-full bg-primary hover:bg-dark-teal disabled:opacity-50 text-white py-5 rounded-2xl font-bold text-lg shadow-xl shadow-primary/20 transition-all flex items-center justify-center gap-3"
                  >
                    <span>Get OTP</span>
                    <ArrowRight size={22} />
                  </button>
              ...
              >
                {isVerified ? (
                  ...
                  <>
                    <h2 className="text-3xl font-bold text-text-dark mb-2 font-display">Verify OTP</h2>
                    <p className="text-text-muted font-medium mb-8">Sent to +91 {phone}</p>
                    ...
                    <div className="flex justify-between gap-2 mb-10">
                      {otp.map((digit, index) => (
                        <input
                          ...
                          className="w-11 h-12 border-2 border-background-light bg-background-light rounded-xl text-center text-xl font-bold text-primary focus:border-primary transition-all outline-none"
                        />
                      ))}
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-primary hover:bg-dark-teal disabled:opacity-50 text-white rounded-2xl font-bold text-lg shadow-xl shadow-primary/20 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <button 
                      onClick={() => setStep(1)} 
                      className="mt-6 text-sm font-bold text-text-muted hover:text-primary transition-colors"
                    >
                      Change Number
                    </button>
                  </>
        ...
        <div className="mt-8 p-6 bg-primary/5 rounded-3xl border border-primary/10 flex items-start gap-4">
          ...
          <p className="text-xs text-text-muted leading-relaxed font-medium text-left">
